...
| `request` | `object` | ❌ | HTTP 请求默认值（`baseURL`/`timeout`/`headers`），见下与 [第 4.5 节](#45-网络请求)。 |
...
声明插件发起 HTTP 请求时的默认值。主进程侧（`ctx.http`）和 UI 侧（`pluginHost.request` / `createClient`）都会用到。每个字段都可在创建客户端或单次请求时被覆盖。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `baseURL` | `string` | 相对路径请求的基地址前缀，如 `https://api.example.com`。 |
| `timeout` | `number` | 单次请求超时（毫秒）；省略或 `0` 表示不超时。 |
| `headers` | `Record<string, string>` | 合并进每次请求的默认请求头。 |
...
```json
{
  "request": {
    ...
    "timeout": 15000,
    "headers": { "X-From": "orccode-plugin" }
  }
}
```

详见 [第 4.5 节：网络请求](#45-网络请求)。
...
| `request(config) → Promise<any>` | 发起 HTTP 请求（实际 fetch 在主进程执行）。见 [第 4.5 节](#45-网络请求)。 |
| `createClient(defaults?) → client` | 创建绑定了 `baseURL` 等默认值的小客户端，含 `get/post/put/patch/delete/request`。 |
...
- **不能**引用 CDN 脚本/样式、外链字体、远程图片，也**不能在 UI 里直接 `fetch` 外部地址**。所有静态资源需放在插件目录内。
- 但 UI **可以发起 HTTP 请求** —— 通过 `pluginHost.request` / `createClient`，实际的 fetch 在**主进程**执行（不受本 CSP 约束），见 [第 4.5 节](#45-网络请求)。
...
插件可以请求后端接口。无论从**主进程侧**（`ctx.http`）还是**UI 侧**（`pluginHost.request` / `createClient`）发起，实际的 fetch 都在**主进程**用 Electron 的 `net.fetch` 执行（走 Chromium 网络栈、识别系统代理）。这样 UI 在沙箱 CSP 下也能联网（请求并非在 webview 里直接发出，而是经 IPC 桥转交主进程）。

- `baseURL` 默认取清单 `request.baseURL`，可在创建客户端或单次请求时覆盖 —— 即**每个插件可自定义自己的 base_url**。
- 默认返回**解析后的响应体**（JSON 自动解析，否则为文本）；传 `responseReturn: 'raw'` 可拿到含 `status`/`headers` 的完整响应。
- 纯对象的 `data` 会自动 `JSON.stringify` 并补 `Content-Type: application/json`。
- 实现见 `src/main/plugins/http.ts` 与共享引擎 `src/shared/request-client.ts`。

> 请求配置 `config` 字段：`url`、`method`、`baseURL`、`headers`、`params`（查询参数）、`data`（请求体）、`timeout`、`responseReturn`。

**主进程侧** —— 通过 `init(ctx)` 拿到的 `ctx.http`（已绑定清单 `request` 默认值）：

```js
let http; // 在 init 时拿到，供 rpc 使用

module.exports = {
  async init(ctx) {
    http = ctx.http; // 已绑定清单 request.baseURL

    // 直接发请求
    const list = await http.get("/todos", { params: { page: 1 } });
    const created = await http.post("/todos", { title: "hello" });

    // 也可以为另一个后端单独建客户端
    const gh = ctx.createHttpClient({ baseURL: "https://api.github.com" });
    const repo = await gh.get("/repos/anthropics/anthropic-sdk-typescript");
  },

  rpc: {
    // 推荐：把请求封装在 rpc 里，UI 调 invoke("search", { q }) 即可
    async search(args) {
      return await http.get("/search", { params: { q: args.q } });
    },
  },
};
```
...
```js
// 单次请求（baseURL 取清单默认值）
const data = await window.pluginHost.request({
  url: "/todos",
  params: { page: 1 },
});

// POST，body 为对象自动按 JSON 发送
await window.pluginHost.request({
  url: "/todos",
  method: "POST",
  data: { title: "hello" },
});

// 绑定一个自定义 baseURL 的客户端
const api = window.pluginHost.createClient({ baseURL: "https://api.example.com" });
const me = await api.get("/user/me");
const raw = await api.get("/ping", { responseReturn: "raw" }); // 拿到 status/headers
```

> 失败时（网络错误、超时、HTTP 非 2xx）会 reject，请用 `try/catch` 处理。请求与响应数据需可 JSON 序列化（跨 IPC）。
...
- **view 插件 UI = 沙箱**。UI 无 Node、无 `require`，不能直接 `fetch` 外部地址，只能经 `pluginHost` 桥与主进程对话；任何特权操作都必须显式放进 `rpc` 白名单。
- **网络请求经主进程转交**。UI 的 `pluginHost.request` 在主进程执行 fetch，可访问任意地址；请只安装可信插件，并对外发数据保持谨慎。
...
| 插件 HTTP 能力 | `src/main/plugins/http.ts` |
| 请求引擎（共享） | `src/shared/request-client.ts` |
